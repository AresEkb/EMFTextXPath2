		...
		for (EStructuralFeature feature : features) {
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.emftext.language.xpath2.resource.xpath2.util.Xpath2ListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (value instanceof List<?>) {
						List<String> list = org.emftext.language.xpath2.resource.xpath2.util.Xpath2ListUtil.castListUnchecked(value);
