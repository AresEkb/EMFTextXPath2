...
RULES {
	...
	OrExpr ::= operand ("or" operand)+;
	AndExpr ::= operand ("and" operand)+;
	ComparisonExpr ::= left operator right;
	RangeExpr ::= from "to" to;
	AdditiveExpr ::= operand (operator[addition : "+", subtraction : "-"] operand)+;
	MultiplicativeExpr ::= operand (operator[multiplication : "*", div : "div", idiv : "idiv", mod : "mod"] operand)+;
	UnionExpr ::= operand (operation[union : "union", vertical_bar : "|"] operand)+;
	IntersectExceptExpr ::= operand (operator[intersect : "intersect", except : "except"] operand)+;
	InstanceofExpr ::= operand "instance" "of" type;
	TreatExpr ::= operand "treat" "as" type;
	CastableExpr ::= operand "castable" "as" type;
	CastExpr ::= operand "cast" "as" type;
	UnaryExpr ::= operator[plus : "+", minus : "-"]* operand;
	...
	PathExpr ::= initialSeparator:SinglePathSeparator path? | initialSeparator:DoublePathSeparator path | path; 
	SinglePathSeparator ::= "/";
	DoublePathSeparator ::= "//";
	RelativePathExpr ::= step (separator step)*;
